...
public interface IBase extends IConst{
	...
	class BASE {
		...
		static {
			for (int from = 0; from < 64; from++) {
				KING_MOVES[from] = getKingMoves(from, KING, NOCASTLE_WHITEKING
						| NOCASTLE_WHITEQUEEN|HALFMOVES);
				KING_MOVES_BLACK[from] = getKingMoves(from, BLACK_KING, NOCASTLE_BLACKKING
						| NOCASTLE_BLACKQUEEN|HALFMOVES);
			}
		}
